...
def push_metrics(glue_job_run_id, glue_job_name='AvroFromS3ToHudiGlueJob', operation='INSERT', grp_cst_mtrcPDF=None, unit='None', metric_name='HudiCRUDMetric', metric_namespace='HudiDataOperations', hudi_table_name='farm', cloudwatch=None, glue_logger=None):
    metric_data = [
            {
    ...
    glue_logger.info("Hudi operation metrics registered.")
...
def release_lock_if_exists(s3, bucket_name, prefix, glue_job_run_id, glue_logger):
    try:
        glue_logger.info(f"Checking if a lock file does exist in '{bucket_name}' bucket")
        s3.Object(bucket_name, prefix).load()
        glue_logger.info("Removing the lock file if issued by the current job")
        for obj in s3.Bucket(bucket_name).objects.filter(Prefix=prefix):
                body = obj.get()['Body'].read()
                if body == glue_job_run_id.encode('ascii'):
                    glue_logger.info("Removing the lock file")
                    s3.Object(bucket_name, prefix).delete()
                    glue_logger.info("The lock file was removed.")
                    return "Success"
    except ClientError as e:
        if e.response['Error']['Code'] == "404":
            return "Success" 
        glue_logger.error(f"Error encountered operating with s3 objects: {str(e)}")
        return "Failure"
    except Exception as e:
        glue_logger.error(f"Error encountered while removing lock file: {str(e)}") 
        return "Failure"
...
def remove_tz_wordings(datetime_value):
...
def getAllTuples(SourcePDF, schema_obj, avro_file_path, glue_logger): 
    list_of_all_tuples = []
            ...
            if field["parent"] is None:
                if field["fieldName"] == "created_at":
                    if schema_obj[0]["fieldName"] == "dateTimeOccurred" and 'dateTimeFormat' in schema_obj[0].keys():
                        tpl += (str(str(dt.strptime(remove_tz_wordings(SourcePDF['dateTimeOccurred'][idx]), schema_obj[0]["dateTimeFormat"]))[:10]), ) 
                    else:    
                    ...
                    if field["fieldName"] in all_columns_list:
                        if pd.isna(SourcePDF[field["fieldName"]][idx]):
                            ...
                            if "dateTimeFormat" in field.keys():
                                tpl += (dt.strptime(remove_tz_wordings(SourcePDF[field["fieldName"]][idx]), field["dateTimeFormat"]), )
                            else:
...
def getUpsertTuplesFlat(SourcePDF, schema_obj, pk_field_name, glue_logger):
    list_of_tuples = []
    ...
    for idx in range(SourcePDF.shape[0]):
        ...
        for field in schema_obj:
            ...
            if "isMergedTo" not in field.keys():
                if pk_field_name == fieldName and (SourcePDF[fieldName][idx] is None or str(SourcePDF[fieldName][idx]) == ""):
                ...
                if SourcePDF[fieldName][idx] is None:
                    ...
                    tpl += (bool(SourcePDF[fieldName][idx]), )
                elif field["type"] == "TimestampType":
...
schema, list_of_fields, list_of_fields_flat = get_schema(schema_obj)  
...
try:
    ...
    while len(list(s3.Bucket(notification_bucket_name).objects.filter(Prefix=notification_prefix))) > 1:
        ...
        for file_name in files_to_process_ordered: 
            ...
            if len(notification_file_name) > 0: 
                ...
                if not ready_for_hudi_processing:
                    ...
                    list_of_all_tuples = getAllTuples(AllPDF, schema_obj, file_path, logger) 
        ...
        list_of_insert_tuples = getUpsertTuplesFlat(insertPDF, schema_obj, hudi_record_key, logger)
        
        logger.info("Creating list of update tuples")
        list_of_update_tuples = []
        list_of_update_tuples = getUpsertTuplesFlat(updatePDF, schema_obj, hudi_record_key, logger)
        ...
        if insrtDyF.count() > 0:
            ...
            push_metrics(glue_job_run_id, glue_job_name, 'INSERT', cst_mtrcPDF, 'None', glue_metric_name, glue_metric_namespace, hudi_table_name, cw_client, logger)
        ...
        if updtDyF.count() > 0:
            ...
            push_metrics(glue_job_run_id, glue_job_name, 'UPDATE', cst_mtrcPDF, 'None', glue_metric_name, glue_metric_namespace, hudi_table_name, cw_client, logger)
        ...
        if dltDyF.count() > 0:
            ...
            push_metrics(glue_job_run_id, glue_job_name, 'DELETE', cst_mtrcPDF, 'None', glue_metric_name, glue_metric_namespace, hudi_table_name, cw_client, logger)
    ...
    logger.error(str(e))
    release_lock_if_exists(s3, bucket_name, prefix, glue_job_run_id, logger)
