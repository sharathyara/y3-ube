...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred", "id", "seasonName", "startDate","endDate", "cropId", "expectedYield", "expectedYieldUOMId", "cropDescriptionId", "additionalParameters", "createdBy", "createdDateTime", "modifiedBy", "modifiedDateTime", "fieldId", "farmId", "fieldName", "fieldSize", "fieldSizeUOMId", "feature", "geometryHash", "eventSource", "eventType", "eventId", "created_at").filter("created_at >= '2022-09-05'")
