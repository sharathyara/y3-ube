...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred", "id", "userName", "createdBy","onboarded","clientCode","createdDateTime","modifiedBy", "modifiedDateTime","eventSource","eventType","eventId","created_at").withColumnRenamed("id", "userId").filter("created_at >= '2022-09-05'")
