...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred", "sourceFile_name", "sourceFile_receivedDate", "sourceFarmId","sourceUserId", "farmName", "address", "countryId", "regionId", "zipcode", "farmSize", "farmSizeUOMId", "noOfFields", "noOfFarmHands","clientCode","eventSource", "eventType", "eventId", "created_at").filter("created_at >= '2022-09-05'")
