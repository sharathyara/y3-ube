...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred", "sourceFile_name", "sourceFile_receivedDate", "email", "phone", "sourceUserId","type","clientCode","deletionRequestId","name", "countryId", "regionId", "languagePreference", "timezone", "marketingConsent", "dataConsent", "otherConsent", "businessEntity", "postalAddress", "role", "eventSource", "eventType", "eventId", "created_at").filter("created_at >= '2022-09-05'")
