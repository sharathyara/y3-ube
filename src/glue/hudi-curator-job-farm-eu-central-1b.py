...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred", "farmId", "farmName", "userId","organizationId", "regionId", "countryId", "address", "zipcode", "noOfFarmHands", "farmSizeUOMId", "createdBy", "createdDateTime", "modifiedBy", "modifiedDateTime", "eventSource", "eventType", "eventId", "created_at").filter("created_at >= '2022-09-05'")
