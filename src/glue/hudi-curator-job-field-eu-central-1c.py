...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred", "fieldId", "fieldName", "userId","organizationId","farmId", "regionId", "countryId", "fieldSize", "changedFieldSize", "fieldSizeUOMId", "feature", "center", "geoHash", "geometryHash", "createdBy", "createdDateTime", "modifiedBy", "modifiedDateTime", "eventSource", "eventType", "eventId", "created_at").withColumn("feature", replace(col("feature"), "*****")).filter("created_at >= '2022-09-05'")


dataSourceCuratedDF = dataSourceQueryDF.na.fill({"feature": "{}"})	

logger.info(log_former.get_message(f'{hudi_table_name} Schema'))

try:
    if dataSourceCuratedDF.count() > 0:
        ...
        dataSourceCuratedDF.write.format("hudi") \
