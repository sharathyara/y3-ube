...
args = getResolvedOptions(sys.argv,
                          ['JOB_NAME', 'conformed_bucket', 'curated_bucket', 'hudi_db_name', 'hudi_table_name',
                           'hudi_target_table_name', 'hive_target_table_name', 'hudi_target_db_name',
                           'hudi_record_key_name'], )
...
# fieldList = ["request_configuration"]
...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load(
    's3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select(
    "dateTimeOccurred",
    "eventId",
    "request_boundaries",
    "request_field_area",
    "request_field_areaUnitId",
    "request_configuration",
    "request_cropRegionId",
    "request_date",
    "request_extrapolation",
    "response_success",
    "response_message",
    "response_payload_imageUrl",
    "response_payload_cloudCoverage",
    "response_payload_snowCoverage",
    "response_payload_valid",
    "response_payload_s1Max",
    "response_payload_growthScaleId",
    "response_payload_fromGrowthStageId",
    "response_payload_toGrowthStageId",
    "response_payload_date",
...
try:
    if dataSourceQueryDF.count() > 0:
        ...
        additional_options = hudiparammaper.get_hudi_config(destination_bucket_name, hudi_target_db_name,
                                                            hudi_target_table_name, hive_target_table_name,
                                                            hudi_record_key_name, hudi_precombine_field,
