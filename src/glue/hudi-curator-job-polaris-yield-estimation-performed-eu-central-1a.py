...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred","boundaries", "cropRegionId", "request_date", "area", "areaUnitId", "requestType", "callbackUrl", "extrapolation", "id", "success", "begin", "end", "message", "min", "max", "mean", "std", "cloudCoverage", "snowCoverage", "valid", "image_date", "solution", "eventSource", "eventType", "eventId", "created_at").filter("created_at >= '2022-09-05'")
