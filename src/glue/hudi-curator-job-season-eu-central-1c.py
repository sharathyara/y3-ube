...
dataSourceQueryDF = spark.read.format('org.apache.hudi').load('s3://' + source_bucket_name + f'/hudi-cdc-tables/{hudi_table_name}' + '/*/*').select("dateTimeOccurred", "id", "seasonName", "startDate","endDate", "cropId", "fieldId", "farmId", "fieldName", "fieldSize", "expectedYield", "expectedYieldUOMId", "cropDescriptionId", "additionalParameters",  "fieldSizeUOMId", "feature", "geometryHash", "createdBy", "createdDateTime", "modifiedBy", "modifiedDateTime", "eventSource", "eventType", "eventId", "created_at").withColumn("feature", replace(col("feature"), "*****")).filter("created_at >= '2022-09-05'")
...
logger.info(log_former.get_message(f'Saving records to Hudi'))
try:
