from ffdputils import cwmetrics, hudiparammaper, lock, mapziphandler, metastore, schemaparser, FFDPLogger
...
args = getResolvedOptions(sys.argv, ['JOB_NAME', 'conformed_bucket', 'hudi_db_name', 'hudi_table_name', 'glue_metric_namespace', 'glue_metric_name', 'batch_max_limit', 'message_event_types', 'data_schema_location', 'hudi_record_key', 'hudi_partition_field', 'hudi_precombine_field', 'metastore_table_name', 's3_object', 'map_bucket_name', 'map_extraction_folder_name'])
...
map_extraction_folder = args['map_extraction_folder_name']
...
zip_source_attribute_name = "sourceFile_name"
...
try:
    ...
    while len(list(s3.Bucket(notification_bucket_name).objects.filter(Prefix=notification_prefix))) > 1:
        ...
        for file_name in files_to_process_ordered: 
            ...
            if len(notification_file_name) > 0: 
                ...
                if not ready_for_hudi_processing:
                    ...
                    mapziphandler.extract_map_zip(s3, map_bucket_name, map_extraction_folder, notification_lock_file_name, zip_source_attribute_name, allProcessedDF, glue_job_run_id, logger, log_former)
        ...
        if upsertDF.count() > 0:
            ...
            metaDataDF = upsertDF[[hudi_precombine_field, hudi_record_key, zip_source_attribute_name, 'eventType', list_of_fields_flat[-1]]]
            metastore.put_to_metastore(entityTable, metaDataDF, hudi_record_key, hudi_precombine_field, hudi_table_name, list_of_fields_flat, keyword_delete, logger, log_former, zip_source_attribute_name)
        ...
        if deleteDF.count() > 0:
            ...
            metaDataDF = deleteDF[[hudi_precombine_field, hudi_record_key, zip_source_attribute_name, 'eventType', list_of_fields_flat[-1]]]
            metastore.put_to_metastore(entityTable, metaDataDF, hudi_record_key, hudi_precombine_field, hudi_table_name, list_of_fields_flat, keyword_delete, logger, log_former, zip_source_attribute_nam)
