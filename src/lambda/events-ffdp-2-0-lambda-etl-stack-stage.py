...
def lambda_handler(event, context):
    ...
    for msg in event['Records']:
        ...
        try:
            body = ""
            for obj in s3.Bucket(s3_full_path.split("/")[2]).objects.filter(Prefix=f"parameters/{entity_name}_parameters.json"):
            ...
            if e.response['Error']['Code'] == "404":
                ...
                return {
                    'statusCode': 404,
                    'body': json.dumps(f"File {entity_name}_parameters.json was not found")
                }
        ...
        prefix = f"{hudi_table_name}/processing-list{processing_list_no}/"
